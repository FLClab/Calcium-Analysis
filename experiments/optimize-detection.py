...
def optimize_detection_threshold(model):
    if os.path.isfile(source_qc_folder):
        valid_data = MSCTDataset(source_qc_folder, ["valid"], return_full=True)
    else:
    ...
    valid_generator = LoadedEvent3D(valid_data, transforms=None)
    
    threshold = model.optimize_threshold_detection(
        valid_generator,
    ...
    with open(os.path.join(prediction_model_path, prediction_model_name, "optimized-threshold-detection"), "w") as file:
        file.write(str(threshold))
    print(f"The optimal threshold is: {threshold:0.3f}")
    
def quality_control(model):
    if os.path.exists(prediction_model_path+"/"+prediction_model_name+"/Quality Control Detection/Prediction"):
        shutil.rmtree(prediction_model_path+"/"+prediction_model_name+"/Quality Control Detection/Prediction")
...
#         "seg-f1-score" : f1_score(truth.ravel(), binary_prediction.ravel()),        
        ...
        df.to_csv(os.path.join(prediction_model_path, prediction_model_name, "Quality Control Detection/QC_metrics_"+prediction_model_name+".csv"))
